...
export class Guard098 {
  ...
  describe() {
    return { name:"Guard098", subject:"batchSize", minimum:this.minimum, purpose:"batch size" };
  }
}

export function validateGuard098(value:number, minimum=31) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard098(value:number, minimum=31) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard098(value:number, minimum=31) {
