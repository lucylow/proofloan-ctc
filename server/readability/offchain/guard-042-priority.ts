...
export class Guard042 {
  ...
  describe() {
    return { name:"Guard042", subject:"priority", minimum:this.minimum, purpose:"priority" };
  }
}

export function validateGuard042(value:number, minimum=16) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard042(value:number, minimum=16) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard042(value:number, minimum=16) {
