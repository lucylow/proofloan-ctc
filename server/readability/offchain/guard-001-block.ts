...
export class Guard001 {
  ...
  describe() {
    return { name:"Guard001", subject:"block", minimum:this.minimum, purpose:"block number" };
  }
}

export function validateGuard001(value:number, minimum=8) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard001(value:number, minimum=8) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard001(value:number, minimum=8) {
