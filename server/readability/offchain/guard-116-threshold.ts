...
export class Guard116 {
  ...
  describe() {
    return { name:"Guard116", subject:"threshold", minimum:this.minimum, purpose:"threshold" };
  }
}

export function validateGuard116(value:number, minimum=29) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard116(value:number, minimum=29) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard116(value:number, minimum=29) {
