...
export class Guard018 {
  ...
  describe() {
    return { name:"Guard018", subject:"index", minimum:this.minimum, purpose:"index" };
  }
}

export function validateGuard018(value:number, minimum=10) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard018(value:number, minimum=10) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard018(value:number, minimum=10) {
