...
export class Guard034 {
  ...
  describe() {
    return { name:"Guard034", subject:"confirmations", minimum:this.minimum, purpose:"confirmation count" };
  }
}

export function validateGuard034(value:number, minimum=18) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard034(value:number, minimum=18) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard034(value:number, minimum=18) {
