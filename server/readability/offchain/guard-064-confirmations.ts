...
export class Guard064 {
  ...
  describe() {
    return { name:"Guard064", subject:"confirmations", minimum:this.minimum, purpose:"confirmation count" };
  }
}

export function validateGuard064(value:number, minimum=25) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard064(value:number, minimum=25) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard064(value:number, minimum=25) {
