...
export class Guard092 {
  ...
  describe() {
    return { name:"Guard092", subject:"cursor", minimum:this.minimum, purpose:"cursor" };
  }
}

export function validateGuard092(value:number, minimum=30) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard092(value:number, minimum=30) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard092(value:number, minimum=30) {
