...
export class Guard049 {
  ...
  describe() {
    return { name:"Guard049", subject:"logIndex", minimum:this.minimum, purpose:"log index" };
  }
}

export function validateGuard049(value:number, minimum=18) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard049(value:number, minimum=18) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard049(value:number, minimum=18) {
