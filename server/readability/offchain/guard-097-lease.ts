...
export class Guard097 {
  ...
  describe() {
    return { name:"Guard097", subject:"leaseMs", minimum:this.minimum, purpose:"lease duration" };
  }
}

export function validateGuard097(value:number, minimum=30) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard097(value:number, minimum=30) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard097(value:number, minimum=30) {
