...
export class Guard078 {
  ...
  describe() {
    return { name:"Guard078", subject:"index", minimum:this.minimum, purpose:"index" };
  }
}

export function validateGuard078(value:number, minimum=24) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard078(value:number, minimum=24) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard078(value:number, minimum=24) {
