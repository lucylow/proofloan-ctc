...
export class Guard095 {
  ...
  describe() {
    return { name:"Guard095", subject:"range", minimum:this.minimum, purpose:"range" };
  }
}

export function validateGuard095(value:number, minimum=28) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard095(value:number, minimum=28) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard095(value:number, minimum=28) {
