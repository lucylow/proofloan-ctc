...
export class Guard008 {
  ...
  describe() {
    return { name:"Guard008", subject:"batchSize", minimum:this.minimum, purpose:"batch size" };
  }
}

export function validateGuard008(value:number, minimum=10) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard008(value:number, minimum=10) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard008(value:number, minimum=10) {
