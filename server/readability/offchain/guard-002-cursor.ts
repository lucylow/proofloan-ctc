...
export class Guard002 {
  ...
  describe() {
    return { name:"Guard002", subject:"cursor", minimum:this.minimum, purpose:"cursor" };
  }
}

export function validateGuard002(value:number, minimum=9) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard002(value:number, minimum=9) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard002(value:number, minimum=9) {
