...
export class Guard006 {
  ...
  describe() {
    return { name:"Guard006", subject:"deadlineMs", minimum:this.minimum, purpose:"deadline" };
  }
}

export function validateGuard006(value:number, minimum=8) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard006(value:number, minimum=8) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard006(value:number, minimum=8) {
