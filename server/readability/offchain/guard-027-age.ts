...
export class Guard027 {
  ...
  describe() {
    return { name:"Guard027", subject:"ageMs", minimum:this.minimum, purpose:"age" };
  }
}

export function validateGuard027(value:number, minimum=9) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard027(value:number, minimum=9) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard027(value:number, minimum=9) {
