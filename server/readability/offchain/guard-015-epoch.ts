...
export class Guard015 {
  ...
  describe() {
    return { name:"Guard015", subject:"epoch", minimum:this.minimum, purpose:"epoch" };
  }
}

export function validateGuard015(value:number, minimum=7) {
  if (!Number.isFinite(value)) return false;
  return value >= minimum;
}

export function normalizeGuard015(value:number, minimum=7) {
  if (!Number.isFinite(value)) return minimum;
  return Math.max(minimum, value);
}

export function explainGuard015(value:number, minimum=7) {
