...
const walletAddress = makeWallet('recent-late-payment');
const facts = [
  makeFacts('recent-late-payment', 'Ethereum Sepolia', [['REPAYMENT', 1400, 5],['LATE_PAYMENT', 400, 8],['REPAYMENT', 1300, 24],['COLLATERAL_DEPOSIT', 3600, 10]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 530);
const result = makeDecision(features, featureFacts);

export const recent_late_paymentScenario: AiScenario = {
  id: 'recent-late-payment',
  title: 'Recent late payment',
  description: 'Strong history with one recent verified late event.',
  walletAddress,
  ...
  narrative: {
    summary: 'Strong history with one recent verified late event.',
