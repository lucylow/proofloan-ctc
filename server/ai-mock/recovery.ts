...
const walletAddress = makeWallet('recovery');
const facts = [
  makeFacts('recovery', 'Ethereum Sepolia', [['LATE_PAYMENT', 600, 75],['REPAYMENT', 1100, 30],['REPAYMENT', 1300, 12],['REPAYMENT', 1500, 3],['COLLATERAL_DEPOSIT', 3500, 6]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 790);
const result = makeDecision(features, featureFacts);

export const recoveryScenario: AiScenario = {
  id: 'recovery',
  title: 'Recovery pathway',
  description: 'A previous issue is followed by improving fresh evidence.',
  walletAddress,
  ...
  narrative: {
    summary: 'A previous issue is followed by improving fresh evidence.',
