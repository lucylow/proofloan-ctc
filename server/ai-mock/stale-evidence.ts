...
const walletAddress = makeWallet('stale-evidence');
const facts = [
  makeFacts('stale-evidence', 'Polygon Amoy', [['REPAYMENT', 1500, 140],['REPAYMENT', 900, 180],['COLLATERAL_DEPOSIT', 3200, 220]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 1200);
const result = makeDecision(features, featureFacts);

export const stale_evidenceScenario: AiScenario = {
  id: 'stale-evidence',
  title: 'Stale evidence',
  description: 'Evidence exists but is old relative to the decision window.',
  walletAddress,
  ...
  narrative: {
    summary: 'Evidence exists but is old relative to the decision window.',
