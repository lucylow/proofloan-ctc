...
const walletAddress = makeWallet('new-wallet');
const facts = [
  makeFacts('new-wallet', 'Ethereum Sepolia', [['REPAYMENT', 600, 1],['COLLATERAL_DEPOSIT', 1200, 2]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 18);
const result = makeDecision(features, featureFacts);

export const new_walletScenario: AiScenario = {
  id: 'new-wallet',
  title: 'New wallet',
  description: 'Very recent wallet activity with little history.',
  walletAddress,
  ...
  narrative: {
    summary: 'Very recent wallet activity with little history.',
