...
const walletAddress = makeWallet('collateral-heavy');
const facts = [
  makeFacts('collateral-heavy', 'Polygon Amoy', [['COLLATERAL_DEPOSIT', 9000, 3],['REPAYMENT', 1200, 7],['REPAYMENT', 1300, 15]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 740);
const result = makeDecision(features, featureFacts);

export const collateral_heavyScenario: AiScenario = {
  id: 'collateral-heavy',
  title: 'Collateral heavy',
  description: 'Strong collateral coverage with moderate repayment history.',
  walletAddress,
  ...
  narrative: {
    summary: 'Strong collateral coverage with moderate repayment history.',
