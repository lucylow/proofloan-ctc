...
const walletAddress = makeWallet('aging-evidence');
const facts = [
  makeFacts('aging-evidence', 'Ethereum Sepolia', [['REPAYMENT', 1000, 35],['REPAYMENT', 1100, 44],['COLLATERAL_DEPOSIT', 2400, 52]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 840);
const result = makeDecision(features, featureFacts);

export const aging_evidenceScenario: AiScenario = {
  id: 'aging-evidence',
  title: 'Aging evidence',
  description: 'Useful but aging historical activity.',
  walletAddress,
  ...
  narrative: {
    summary: 'Useful but aging historical activity.',
