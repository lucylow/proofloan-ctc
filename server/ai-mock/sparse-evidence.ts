...
const walletAddress = makeWallet('sparse-evidence');
const facts = [
  makeFacts('sparse-evidence', 'Ethereum Sepolia', [['REPAYMENT', 700, 2]])
];
const featureFacts = facts.flat();
const features = featuresFromFacts(featureFacts, 42);
const result = makeDecision(features, featureFacts);

export const sparse_evidenceScenario: AiScenario = {
  id: 'sparse-evidence',
  title: 'Sparse evidence',
  description: 'Only a single recent event is available for model interpretation.',
  walletAddress,
  ...
  narrative: {
    summary: 'Only a single recent event is available for model interpretation.',
