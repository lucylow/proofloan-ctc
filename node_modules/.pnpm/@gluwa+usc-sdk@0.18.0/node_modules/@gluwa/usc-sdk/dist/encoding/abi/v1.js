 ...
 */
function encodeCommonFields(tx) {
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    return coder.encode(['uint64', 'uint64', 'address', 'bool', 'address', 'uint256', 'bytes'], [tx.nonce, tx.gasLimit, tx.from, tx.to == null, (0, utils_1.addressOrZero)(tx.to), tx.value, tx.data]);
}
/**
 ...
 */
function encodeReceiptFields(rx) {
    var _a;
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    // For status field, default to 1 (success) if undefined (pre-EIP-658 receipts)
    // See: https://eips.ethereum.org/EIPS/eip-658
    return coder.encode(['uint8', 'uint64', 'tuple(address, bytes32[], bytes)[]', 'bytes'], [(_a = rx.status) !== null && _a !== void 0 ? _a : 1, rx.gasUsed, rx.logs.map((log) => [log.address, log.topics, log.data]), rx.logsBloom]);
}
/**
 ...
 */
function getChunksForType0(tx, rx) {
    var _a;
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    // Chunk 1: Common fields
    const chunk1 = encodeCommonFields(tx);
    // Chunk 2: Type-specific fields (gasPrice, v, r, s)
    const chunk2 = coder.encode(['uint128', 'uint256', 'bytes32', 'bytes32'], [tx.gasPrice, (_a = tx.signature.networkV) !== null && _a !== void 0 ? _a : tx.signature.v, tx.signature.r, tx.signature.s]);
 ...
 * Chunk 3: Receipt fields (receiptStatus, receiptGasUsed, receiptLogs, receiptLogsBloom)
 */
function getChunksForType1(tx, rx) {
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    // Chunk 1: Common fields
    const chunk1 = encodeCommonFields(tx);
    // Chunk 2: Type-specific fields (chainId, gasPrice, accessList, yParity, r, s)
    const chunk2 = coder.encode(['uint64', 'uint128', 'tuple(address,bytes32[])[]', 'uint8', 'bytes32', 'bytes32'], [tx.chainId, tx.gasPrice, encodeAccessList(tx.accessList), tx.signature.yParity, tx.signature.r, tx.signature.s]);
    // Chunk 3: Receipt fields
    const chunk3 = encodeReceiptFields(rx);
    return [chunk1, chunk2, chunk3];
}
 ...
 * Chunk 3: Receipt fields (receiptStatus, receiptGasUsed, receiptLogs, receiptLogsBloom)
 */
function getChunksForType2(tx, rx) {
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    // Chunk 1: Common fields
    const chunk1 = encodeCommonFields(tx);
    // Chunk 2: Type-specific fields (chainId, maxPriorityFeePerGas, maxFeePerGas, accessList, yParity, r, s)
    const chunk2 = coder.encode(['uint64', 'uint128', 'uint128', 'tuple(address,bytes32[])[]', 'uint8', 'bytes32', 'bytes32'], [
        tx.chainId,
 ...
 */
function getChunksForType3(tx, rx) {
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    // Chunk 1: Common fields
    const chunk1 = encodeCommonFields(tx);
    // Chunk 2: Type-specific fields part 1 (chainId, maxPriorityFeePerGas, maxFeePerGas, accessList)
    const chunk2 = coder.encode(['uint64', 'uint128', 'uint128', 'tuple(address,bytes32[])[]'], [tx.chainId, tx.maxPriorityFeePerGas, tx.maxFeePerGas, encodeAccessList(tx.accessList)]);
    // Chunk 3: Type-specific fields part 2 (maxFeePerChunkGas, blobVersionedHashes, yParity, r, s)
    const chunk3 = coder.encode(['uint256', 'bytes32[]', 'uint8', 'bytes32', 'bytes32'], [tx.maxFeePerBlobGas, tx.blobVersionedHashes, tx.signature.yParity, tx.signature.r, tx.signature.s]);
    // Chunk 4: Receipt fields
    const chunk4 = encodeReceiptFields(rx);
    return [chunk1, chunk2, chunk3, chunk4];
}
function encodeAuthorizationList(authorizationList, rawAuthorizationList) {
 ...
 * Chunk 4: Receipt fields (receiptStatus, receiptGasUsed, receiptLogs, receiptLogsBloom)
 */
function getChunksForType4(tx, raw, rx) {
    const coder = ethers_1.AbiCoder.defaultAbiCoder();
    // Chunk 1: Common fields
    const chunk1 = encodeCommonFields(tx);
    // Chunk 2: Type-specific fields part 1 (chainId, maxPriorityFeePerGas, maxFeePerGas, accessList)
    const chunk2 = coder.encode(['uint64', 'uint128', 'uint128', 'tuple(address,bytes32[])[]'], [tx.chainId, tx.maxPriorityFeePerGas, tx.maxFeePerGas, encodeAccessList(tx.accessList)]);
    // Chunk 3: Type-specific fields part 2 (authorizationList, yParity, r, s)
    const chunk3 = coder.encode(['tuple(uint256,address,uint64,uint8,uint256,uint256)[]', 'uint8', 'bytes32', 'bytes32'], [
...
function getTypesForType(txType) {
    switch (txType) {
        case 0:
            // Chunk 1: Common fields (nonce, gasLimit, from, toIsNull, to, value, data)
            // Chunk 2: Type-specific fields (gasPrice, v, r, s)
 ...
 */
function getChunksForType(tx, rx) {
