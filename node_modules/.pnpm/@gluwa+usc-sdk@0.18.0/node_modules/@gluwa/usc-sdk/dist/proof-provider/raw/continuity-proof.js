...
exports.ContinuityProofBuilder = exports.AttestationBlock = void 0;
const merkle_1 = require("../merkle");
class AttestationBlock {
    constructor(blockNumber, root, digest, prevDigest) {
...
exports.AttestationBlock = AttestationBlock;
class ContinuityProofBuilder {
    constructor(blockProvider, chainInfoProvider, chainKey, encoding) {
    ...
    static createFrom(blocks) {
        ...
        const continuityRoots = blocks.map((b) => b.root);
        return {
            lowerEndpointDigest,
     ...
     * ```
     */
    createForHeights(fromHeight_1) {
        return __awaiter(this, arguments, void 0, function* (fromHeight, toHeight = null) {
            const genesisHeight = yield this.chainInfoProvider.getAttestationGenesisHeight(this.chainKey);
            if (fromHeight < genesisHeight) {
                throw new Error(`Cannot build continuity proof for height ${fromHeight} below attestation genesis height ${genesisHeight}`);
            }
            ...
            const fromBounds = yield this.chainInfoProvider.getContinuityBounds(this.chainKey, fromHeight);
            if (!fromBounds.isAttested) {
                throw new Error(`Cannot build continuity proof for height ${fromHeight} without both lower and upper continuity bounds`);
            }
            console.log(`Found attestation bounds for height ${fromHeight}: lower=${fromBounds.parentHeight}, upper=${fromBounds.childHeight}`);
            if (toHeight !== null) {
                const toBounds = yield this.chainInfoProvider.getContinuityBounds(this.chainKey, toHeight);
                if (!toBounds.isAttested) {
                    throw new Error(`Cannot build continuity proof for height ${toHeight} without both lower and upper continuity bounds`);
                }
                console.log(`Found attestation bounds for height ${toHeight}: lower=${toBounds.parentHeight}, upper=${toBounds.childHeight}`);
                ...
                const blocks = yield this.buildAndTrimContinuityFor(fromHeight, combinedBounds);
                ...
                // Using those bounds build the continuity blocks
                const blocks = yield this.buildAndTrimContinuityFor(fromHeight, fromBounds);
                console.log(`Built ${blocks.length} continuity blocks for height ${fromHeight}`);
                // Finally convert to ContinuityProof
                return ContinuityProofBuilder.createFrom(blocks);
            }
    ...
    buildAndTrimContinuityFor(queryHeight, bounds) {
        return __awaiter(this, void 0, void 0, function* () {
            const latestBlockNumber = yield this.blockProvider.getBlockNumber();
            ...
            if (bounds.childHeight > latestBlockNumber) {
                throw new Error(`Cannot build continuity proof up to attestation/checkpoint at height ${bounds.childHeight} greater than latest block number ${latestBlockNumber}`);
            }
            ...
            const blocks = yield this.buildContinuityBlocks(buildStartHeight, endHeight, bounds.parentHash);
            ...
            if (lastBlock.blockNumber !== endHeight) {
                throw new Error(`Trimmed blocks don't end at upper attestation block: expected ${endHeight}, got ${lastBlock.blockNumber}`);
            }
            return filteredBlocks;
        });
    }
    buildContinuityBlocks(buildStartHeight, endHeight, lowerDigest) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const continuityBlocks = blocksWithReceipt.map(({ block, transactions, receipts }) => {
                ...
                const merkleRoot = (0, merkle_1.computeMerkleRootOfBlock)(orderedTransactions, orderedReceipts, this.encoding);
                const digest = (0, merkle_1.computeDigestOf)(block.number, merkleRoot, prevDigest);
...
exports.ContinuityProofBuilder = ContinuityProofBuilder;
