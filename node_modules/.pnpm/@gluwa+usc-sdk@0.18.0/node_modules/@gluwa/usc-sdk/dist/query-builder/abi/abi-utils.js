...
exports.computeAbiOffsets = computeAbiOffsets;
const ethers_1 = require("ethers");
const ForkedReader_1 = require("../common/ForkedReader");
const is_dynamic_1 = require("../common/is-dynamic");
exports.WORD_SIZE = 32;
/**
 * Computes ABI field offsets by encoding, decoding, and analyzing offsets recursively.
 */
function computeAbiOffsets(paramTypes, encodedData) {
    const reader = new ForkedReader_1.ForkedReader(encodedData, false);
    return decodeRecursive(reader, paramTypes);
}
function decodeRecursive(reader, paramTypes, baseOffset = 0) {
    let result = [];
    ...
    for (let i = 0; i < paramTypes.length; i++) {
        ...
        const field = {
            ...
            isDynamic: (0, is_dynamic_1.isDynamic)(currentParam),
            children: [],
        };
        if (currentParam.isTuple()) {
            if ((0, is_dynamic_1.isDynamic)(currentParam)) {
                const value = (0, ethers_1.hexlify)(reader.readBytes(exports.WORD_SIZE));
                const offset = (0, ethers_1.toNumber)(value);
                const subReader = reader.subReaderAbsolute(offset);
                field.children = decodeRecursive(subReader, currentParam.components, offset);
            }
            else {
                // if you have a tuple
                ...
                field.children = decodeRecursive(reader, currentParam.components, baseOffset);
            }
        }
            ...
            if (currentParam.isArray()) {
                ...
                field.value = (0, ethers_1.hexlify)(reader.readBytes(exports.WORD_SIZE));
                field.size = exports.WORD_SIZE;
            }
        }
        else {
            const value = (0, ethers_1.hexlify)(reader.readBytes(exports.WORD_SIZE));
            const fieldDynamicOffset = (0, ethers_1.toNumber)(value);
            ...
            if (currentParam.isArray()) {
                ...
                if (currentParam.arrayLength !== -1) {
                    ...
                    if ((0, is_dynamic_1.isDynamic)(currentParam.arrayChildren)) {
                        const dynamicArrayRelativeOffsets = [];
                        for (let i = 0; i < numberOfElements; i++) {
                            dynamicArrayRelativeOffsets.push((0, ethers_1.toNumber)(subReader.readBytes(exports.WORD_SIZE)));
                        }
                        // now we can start decoding the children :)
                        const sizeOfOffsets = numberOfElements * exports.WORD_SIZE;
                        field.children = dynamicArrayRelativeOffsets.map((arrayElementOffset, index) => {
                            ...
                            if (currentParam.arrayChildren.type === 'bytes' || currentParam.arrayChildren.type === 'string') {
                                ...
                                const absoluteDataOffset = baseOffset + fieldDynamicOffset + exports.WORD_SIZE + arrayElementOffset;
                                ...
                                return {
                                    ...
                                    offset: absoluteDataOffset + exports.WORD_SIZE, // Absolute position of data start (after length prefix)
                                    isDynamic: true,
                                    size: length,
                                    value: (0, ethers_1.hexlify)(dataSubReader.readBytes(length)),
                                    children: [],
                                };
                            }
                            ...
                            const position = baseOffset + fieldDynamicOffset + arrayElementOffset + exports.WORD_SIZE;
                            const children = decodeRecursive(arrayItemSubReader, currentParam.arrayChildren.components, position);
                            return {
                                type: currentParam.format('full'),
                                offset: position,
                                isDynamic: true,
                ...
                field.offset = baseOffset + fieldDynamicOffset + exports.WORD_SIZE;
                field.size = length;
                field.value = (0, ethers_1.hexlify)(subReader.readBytes(length));
            }
        }
        result.push(field);
    }
