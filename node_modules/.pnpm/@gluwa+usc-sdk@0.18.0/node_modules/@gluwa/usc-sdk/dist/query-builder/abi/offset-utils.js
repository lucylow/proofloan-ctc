...
exports.makeOffsetsAbsolute = makeOffsetsAbsolute;
exports.computeAllOffsets = computeAllOffsets;
const ethers_1 = require("ethers");
const models_1 = require("./models");
const abi_utils_1 = require("./abi-utils");
const abi_1 = require("../../encoding/abi");
const encoding_mapping_1 = require("./encoding-mapping");
/**
 * Recursively makes offsets absolute by adding baseOffset to field and all children
 */
function makeOffsetsAbsolute(field, baseOffset) {
    var _a;
    return Object.assign(Object.assign({}, field), { offset: field.offset + baseOffset, children: ((_a = field.children) === null || _a === void 0 ? void 0 : _a.map((child) => makeOffsetsAbsolute(child, baseOffset))) || [] });
}
/**
 ...
 */
function computeAllOffsets(tx, rx, encoding = abi_1.EncodingVersion.V1) {
    const result = (0, abi_1.abiEncode)(tx, rx, encoding);
    // Get explicit chunk definitions
    const fieldMapping = (0, encoding_mapping_1.getAllFieldsForTransaction)(tx.formatted.type, encoding);
    ...
    const paramTypes = [ethers_1.ParamType.from('uint8'), ethers_1.ParamType.from('bytes[]')];
    const offsets = (0, abi_utils_1.computeAbiOffsets)(paramTypes, result.abi);
    ...
    if (bytesArrayOffset.children.length !== chunks.length) {
        throw new Error(`Mismatch: computed ${bytesArrayOffset.children.length} chunk offsets but decoded ${chunks.length} chunks`);
    }
    if (fieldMapping.chunks.length !== chunks.length) {
        throw new Error(`Mismatch: field mapping has ${fieldMapping.chunks.length} chunks but encoding produced ${chunks.length} chunks`);
    }
    ...
    for (let chunkIndex = 0; chunkIndex < fieldMapping.chunks.length; chunkIndex++) {
        ...
        const chunkParamTypes = chunkTypes.map((type) => ethers_1.ParamType.from(type));
        const chunkFieldOffsets = (0, abi_utils_1.computeAbiOffsets)(chunkParamTypes, chunk);
    ...
    map.set(models_1.QueryableFields.Type, {
        type: 'uint8',
        offset: abi_utils_1.WORD_SIZE, // First 32 bytes after bytes memory length prefix
        isDynamic: false,
        size: 1,
        children: [],
    ...
    allFields.forEach((field, index) => {
        ...
        const fieldType = ethers_1.ParamType.from(field.type);
        const computedOffsetType = ethers_1.ParamType.from(computedOffset.type);
